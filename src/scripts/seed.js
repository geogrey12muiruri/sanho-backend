...
async function main() {
  ...
  try {
    ...
    for (const manufacturer of manufacturers) {
      ...
      createdManufacturers[manufacturer.name] = created;
    }

    // Create machine models
    console.log('🚜 Creating machine models...');
    const createdMachineModels = {};
    for (const model of machineModels) {
      const manufacturer = createdManufacturers[model.manufacturer];
      const created = await prisma.machineModel.upsert({
        where: {
          name_manufacturerId: {
            name: model.name,
      ...
      createdMachineModels[model.name] = created;
    }
    ...
    for (const part of parts) {
      const category = createdCategories[part.categorySlug];
      const manufacturer = createdManufacturers[part.manufacturerName];
      ...
      // Create part compatibilities
      for (const machineModelName of part.machineModels) {
        const machineModel = createdMachineModels[machineModelName];
        if (machineModel) {
          await prisma.partCompatibility.upsert({
            where: {
              partId_machineModelId: {
